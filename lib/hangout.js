...
(function() {
  ...
  Hangout = (function() {
    function Hangout(gapi1) {
      this.gapi = gapi1;
      this.send_interval = bind(this.send_interval, this);
      ...
      this.gapi.hangout.onApiReady.add((function(_this) {
        return function() {
          _this.gapi.hangout.onair.onBroadcastingChanged.add(_this.changeStatus);
          return _this.notify();
        };
      })(this));
      this.f_send_interval = (function(_this) {
        return function() {
          return _this.send_interval();
        };
      })(this);
      setInterval(this.f_send_interval, 10000);
    }
    ...
    Hangout.prototype.changeStatus = function(e) {
      ...
      if (prev_status !== this.status) {
        ...
        if (this.status === 'finished') {
          clearInterval(this.f_send_interval);
        }
        return this.notify();
      }
    };
    ...
    Hangout.prototype.notify = function() {
      ...
      return $.ajax({
        ...
        success: (function(_this) {
          return function() {
            _this.gapi.hangout.data.setValue('status', 'ok');
            $('#main').append("<p>ajax return: success</p>");
            if (_this.gapi.hangout.data.getValue('updated') !== 'true') {
              _this.gapi.hangout.layout.displayNotice('Connection to WebsiteOne established');
              return _this.gapi.hangout.data.setValue('updated', 'true');
            }
          };
        })(this),
        error: (function(_this) {
          return function() {
            _this.gapi.hangout.data.setValue('status', 'error');
